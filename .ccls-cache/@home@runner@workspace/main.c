...
*Bloco para realizar a compra de veiuculos ofertados no arquivo
veiclos_ofertas.csv

*Sempre que um compra é efetuada os dados do veículo são removidos do arquivo
veiculos_oferta.csv e adicionados em um arquivo "veiculos_estoque.csv" com o
novo preço de venda pretendido

*a compra deve ser registrada no histórico de compras da empresa,no arquivo
"historico_compras.csv". Todo registro de compra deve conter data e hora atual.

*Um arquivo "marcas.csv" deverá ser mantido com as marcas dos veículos em
estoque. Além da  marca,esse arquivo deve armazenar um valor de taxa aplicável,
a partir de 0%, que será aplicado no momento da venda
    ...
    if (arqMarcas == NULL) {
      ...
      while (fgets(linhamarca, 150, arqMarcas) != NULL) {
        ...
        if (strcmp(marcaX.marca, veiculosX.marca) == 0) {
          veiculosX.preco_venda =
              veiculosX.preco_base * (1.0 + marcaX.taxa); // Aplica a taxa
          break;
        }
      }
      ...
      fprintf(estoque, "%s",
              1 + strchr(linha, ',')); // +1 pq a função retorna o ponteiro logo
                                       // em cima da virgula
...
/*
  ...
  Seu sistema também deve permitir a venda de veículos. Sempre que uma venda for
  efetuada, ela deve ser registrada no arquivo historico_vendas.csv e o veículo
  adquirido deve ser removido do arquivo veiculos_estoque.csv. Todo registro de
  venda deve conter data e hora atual.
...
Entrada:
Saida:
Descrição: Altera o dado de um determinado carro a qualquer momento apó ser
adquirido.
*/
...
void AlteracaoDeDados() {
  ...
  case 2:
    ...
    printf("\nVoce escolheu alterar cadastro de um veiculo no estoque,realize "
           "a busca do veiculo que deseja alterar os dados: \n");
    veiculoD = Criafiltro();
    ...
    Asermod = pesquisa(
        veiculoD, "veiculos_estoque.csv",
        5); // acha o veiculo do estoque e armarmzena a linha em asermod
    ...
    adicionarLinha("veiculos_estoque.csv",
                   VeiculoA); // adiciona o veiculo alterado no arquivo
    removerLinha("veiculos_estoque.csv", Asermod);
    break;
  ...
  case 3:
    ...
    adicionarLinhaM("Marcas.csv",
                    marcaA); // adiciona a marca alterada no arquivo
    // removerLinha("Marcas.csv",Asermod2);
    break;
  }
}
...
void relatorioCv() {
  ...
  do {
    system("clear");
    printf(
        "Você deseja exibir o relatório na tela ou escrever em um arquivo?\n");
  ...
  if (op_relatorio == 1) {
    relatorio_file =
        stdout; // stdout é o "arquivo" da saída padrão, que é a tela
  } else {
  ...
  while (fgets(linha, sizeof(linha), file)) {
    if (op == 1) {
      sscanf(linha,
             "%[^,],%[^,],%f,%f,%d,%[^,],%[^,],%[^,],%[^,],%d,%[^,],%[^,],%[^,]"
             ",%[^,],%[^,]",
             veiculo.data, veiculo.hora, &veiculo.preco_venda, &veiculo.preco,
             &veiculo.ano, veiculo.marca, veiculo.modelo, veiculo.condicao,
             veiculo.combustivel, &veiculo.odometro, veiculo.status,
             veiculo.cambio, veiculo.tamanho, veiculo.tipo, veiculo.cor);
    } else if (op == 2) {
      sscanf(linha,
             "%[^,],%[^,],%[^,],%f,%d,%[^,],%[^,],%[^,],%[^,],%d,%[^,],%[^,],%["
             "^,],%[^,],%[^,]",
             veiculo.cliente, veiculo.data, veiculo.hora, &veiculo.preco_venda,
             &veiculo.ano, veiculo.marca, veiculo.modelo, veiculo.condicao,
             veiculo.combustivel, &veiculo.odometro, veiculo.status,
             veiculo.cambio, veiculo.tamanho, veiculo.tipo, veiculo.cor);
    ...
    if (strcmp(veiculo.data, data_inicio_str) >= 0 &&
        strcmp(veiculo.data, data_fim_str) <= 0) {
      ...
      // Chamar a função verelatorio para cada veículo que corresponda ao
      // intervalo de datas
...
2.8 Backup
Para proteger os dados de eventuais falhas, o programa deve efetuar um backup de
todos os arquivos em formato binários quando solicitado pelos usuários, em uma
subpasta chamada “backup”. O programa deverá também permitir a exclusão dos
arquivos de backup.
*/
void Backup() {
...
/*
  ...
  Sempre que uma busca é realizada no sistema, os resultados encontrados devem
  ser exibidos em uma ordem específica. No caso de compras e vendas de veículos,
  deve-se ter uma opção para ordenar pelo (a) veículo mais barato, (b) veículo
  mais caro ou (c) ordenado por marca e modelo em ordem alfabética (ignorando
  maiúsculas e minúsculas), em que veículos da mesma marca são ordenados pelo
  modelo
*/
...
/*____________________________________________________________________________
  ...
  Descrição: abre um arquivo e remove uma linha
      O pricipio de funcionamento é criar um arquvio temporário e copiar pra ele
  tod
*/
...
/*____________________________________________________________________________
    ...
    // preco, ano, marca, modelo, condicao, combustivel, odometro, status,
   cambio, tamanho, tipo, cor
  */
...
/*____________________________________________________________________________
  ...
  // preco, ano, marca, modelo, condicao, combustivel, odometro, status, cambio,
  tamanho, tipo, cor
*/
void printVeiculo(struct veiculo veiculoX, int cabecalho) {
  ...
  printf("[%d] - %s | %s | %d | %.2f | %s \n", cabecalho, veiculoX.marca,
         veiculoX.modelo, veiculoX.ano, veiculoX.preco_base, veiculoX.cor);
}
/*____________________________________________________________________________
  Nome: Print marcas
  Saida: Print no console
...
char *pesquisa(struct veiculo veiculoF, char nome[50], int num_veiculos) {
  ...
  if (arqC == NULL) {
    ...
    while (fgets(linha, 150, arqC) != NULL) {
      ...
      if (((strcmp(veiculoF.marca, "") == 0) ||
           strcmp(veiculoF.marca, veiculoX.marca) == 0) &&
          (veiculoF.ano == 0 || veiculoF.ano == veiculoX.ano) &&
          (veiculoX.preco_base <= veiculoF.preco_base) && linha != NULL) {
    ...
    strcpy(linha,
           linharesposta[cont]); // envia o carro selecionado pra resposta
  }
  fclose(arqC);
  return linha;
}
...
char *pesquisaMarcaTaxa(struct Marca marcas, char nome[50], int num_veiculos) {
  ...
  if (arqC == NULL) {
    ...
    while ((fgets(linha, 150, arqC) != NULL)) {
      ...
      // Se estiver no filtro ele imprime
      if (((strcmp(marcas.marca, "") == 0) ||
           strcmp(marcas.marca, marcaX.marca) == 0) &&
          (marcaX.taxa <= marcas.taxa)) {
...
void adicionarLinha(char *nomeArquivo, struct veiculo veiculo) {
  ...
  if (arquivo == NULL) {
    ...
    fprintf(arquivo, "%0.4f,%d,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s",
            veiculo.preco_base, veiculo.ano, veiculo.marca, veiculo.modelo,
            veiculo.condicao, veiculo.combustivel, veiculo.odometro,
            veiculo.status, veiculo.cambio, veiculo.tamanho, veiculo.tipo,
            veiculo.cor);
  }
...
void ordenabarato(int numlinhas, struct veiculo veiculos[numlinhas]) {
  ...
  while (ordenado !=
         1) {     // Continua repetendo até que não haja nenuma alteração
